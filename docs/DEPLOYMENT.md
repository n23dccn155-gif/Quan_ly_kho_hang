...
| Tên biến (Key) | Giá trị (Value) | Giải thích |
|------|---------|------|
| `NEXT_PUBLIC_API_URL` | `[CHÈN LINK RENDER CỦA BẠN VÀO ĐÂY]/api` | API Endpoint Backend (Render) |
| `BACKEND_URL` | `[CHÈN LINK RENDER CỦA BẠN VÀO ĐÂY]` | Base URL Backend (Không có /api ở cuối) |
...
- **Giải pháp:** Xóa sạch biến cũ trên Settings của Vercel và điền chính xác duy nhất một lần đường link API dạng `[CHÈN LINK RENDER CỦA BẠN VÀO ĐÂY]/api` (không có dấu gạch chéo `/` ở cuối). Tiến hành **Redeploy** để áp dụng thay đổi.
